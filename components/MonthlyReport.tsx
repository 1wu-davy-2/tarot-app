      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-mystic-rose/40 mt-0.5">{monthLabel} · {entryCount} 天记录</p>
          </div>
        </div>
        <ChevronDown className={`w-4 h-4 text-mystic-rose/30 transition-transform ${expanded ? "rotate-180" : ""}`} />
      </button>
            ...
            <div ref={reportRef} className="px-5 pb-5 border-t border-mystic-purple/10">
              {!report && !loading && !error && (
                <div className="pt-5 text-center">
                  <p className="text-xs text-foreground/50 mb-4">AI 将综合分析你本月的日记数据，生成深度趋势报告</p>
                  <button
                  ...
                  {entryCount < 7 && (
                    <p className="text-[10px] text-mystic-rose/30 mt-2">至少需要 7 天记录才能生成月报</p>
                  )}
                </div>
              )}
              ...
              {loading && (
                <div className="pt-5 flex flex-col items-center gap-3">
                  ...
                  <p className="text-xs text-mystic-rose/40">AI 正在深度分析本月数据...</p>
                </div>
              )}
              ...
              {report && (
                <div className="pt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                    {elementChartData.length > 0 && (
                      <div className="glass-card p-3">
                        <p className="text-[10px] text-mystic-rose/40 mb-2 flex items-center gap-1">
                        ...
                        <div className="flex justify-center gap-3 mt-1">
                          {elementChartData.map((d) => (
                            <span key={d.name} className="text-[9px] text-foreground/40" style={{ color: ELEMENT_COLORS[d.name] }}>
                              {d.name} {d.value}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="flex items-center justify-between mt-5 pt-4 border-t border-mystic-purple/10">
                    <button onClick={handleGenerate} disabled={loading} className="flex items-center gap-1.5 text-[10px] text-mystic-rose/40 hover:text-mystic-rose transition-colors disabled:opacity-30">
                      <RefreshCw className={`w-3 h-3 ${loading ? "animate-spin" : ""}`} /> 重新生成
                    </button>
                    <button onClick={() => setExpanded(false)} className="flex items-center gap-1 text-[10px] text-mystic-rose/30 hover:text-mystic-rose/50 transition-colors">
              ...
              {error && (
                <div className="pt-5 text-center">
                  ...
                  <button onClick={handleGenerate} className="text-[10px] text-mystic-rose/40 hover:text-mystic-rose transition-colors">重试</button>
                </div>
              )}
            </div>
          </motion.div>
...
function renderMarkdown(text: string) {
  ...
  return lines.map((line, i) => {
    ...
    if (line.startsWith("- ")) return <li key={i} className="ml-4 text-foreground/70 text-sm">{line.slice(2)}</li>;
    if (/^\d+\. /.test(line)) return <li key={i} className="ml-4 text-foreground/70 text-sm list-decimal">{line.replace(/^\d+\. /, "")}</li>;
    if (line.trim() === "") return <br key={i} />;
    const parts = line.split(/(\*\*[^*]+\*\*)/g);
    if (parts.length > 1) return <p key={i} className="text-sm text-foreground/80 my-1">{parts.map((p, j) => p.startsWith("**") ? <strong key={j} className="text-mystic-gold">{p.slice(2, -2)}</strong> : p)}</p>;
